...
recipe_1 <- recipe(log_price ~ ., data = training_data) |>
  step_mutate(host_listings_ratio = host_listings_count / host_total_listings_count,
              ...
              has_availability = factor(has_availability, levels = c(FALSE, TRUE)), 
              instant_bookable = factor(instant_bookable, levels = c(FALSE, TRUE))) |>
  step_rm(host_listings_count, host_total_listings_count,
          id, host_verifications, host_response_time, first_review_year, last_review_year, 
          has_availability, instant_bookable, reviews_per_month, neighbourhood_cleansed)|>
  step_impute_mean(all_numeric_predictors()) |>
  step_impute_mode(all_nominal_predictors()) |>
